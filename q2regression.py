...
# denominator
dn = np.dot(df['A'], df['A'])*np.dot(df['B'], df['B']) - np.square(np.sum(df['A']*df['B']))
dn

m1 = (np.dot(df['B'], df['B'])*np.dot(df['A'],df['C']) - np.dot(df['A'],df['B'])*np.dot(df['B'],df['C']))/dn
m2 = (np.dot(df['A'], df['A'])*np.dot(df['B'],df['C']) - np.dot(df['A'],df['B'])*np.dot(df['A'],df['C']))/dn            
...
# The two approaches resulting in the same coefficients: b1 = m1 & b2 = m2
